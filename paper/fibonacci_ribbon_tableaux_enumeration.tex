...
premise-free publication endpoints in the accompanying Lean~4
development~\cite{LiFibonacciRibbonLean2026}.
...
The resulting generating-function substitution, together with exact Weyl
integral representations and Regev's strip asymptotics, yields an
...
substitution.  Combining this substitution with exact Weyl integral
representations and Regev's strip asymptotics gives, for every fixed $n\ge3$,
...
\cite{BergeronGascon2000}.  D-finite series are closed under multiplication by a
...
The finite-dimensional Bessel system remains useful for exact recurrences.
Let
\[
...
\]
Writing $d=\lfloor n/2\rfloor$, Gessel's determinant in the simplified form
...
\cite{BergeronGascon2000}.  The identities
\[
 J_0'=2J_1,
 \qquad
 J_1'=2J_0-x^{-1}J_1
\]
give a basis vector $\mathbf F$ satisfying
\begin{equation}\label{eq:bessel-system}
 \mathbf F'(x)=(M_0+x^{-1}M_1)\mathbf F(x),
\end{equation}
where the eigenvalues of $M_0$ are
\begin{equation}\label{eq:bessel-scales}
 n,n-4,n-8,\ldots,n-4d.
\end{equation}
If $\mathbf F=\sum_{k\ge0}\mathbf f_kx^k/k!$ and
$\widehat{\mathbf F}(z)=\sum_{k\ge0}\mathbf f_kz^k$, coefficient comparison
gives
\begin{equation}\label{eq:ordinary-bessel-system}
 z(I-zM_0)\widehat{\mathbf F}'(z)
 =(M_1+zM_0)\widehat{\mathbf F}(z)-M_1\mathbf f_0.
\end{equation}
This system produces the specialized recurrences below.  The asymptotic
argument will instead use the exact Weyl integral carrier, so that its proof
follows the same route as the accompanying formalization.
...
\end{equation}
\cite{Regev1981,Matsumoto2008}.

We now compare two exact Weyl moments.  Put $d=\lfloor n/2\rfloor$ and, for
$\theta=(\theta_1,\ldots,\theta_d)\in[0,\pi]^d$, write
\[
 \Delta_c(\theta)=\prod_{1\le i<j\le d}
   (\cos\theta_j-\cos\theta_i)^2.
\]
For $n=2d$, set
\[
 S_n(\theta)=2\sum_{i=1}^d\cos\theta_i,
 \qquad
 W_n(\theta)=\Delta_c(\theta)\prod_{i=1}^d(1+\cos\theta_i),
 \qquad
 \kappa_n=\frac{2^{d^2-d}}{d!\,\pi^d};
\]
for $n=2d+1$, set
\[
 S_n(\theta)=1+2\sum_{i=1}^d\cos\theta_i,
 \qquad
 W_n(\theta)=\Delta_c(\theta)
   \prod_{i=1}^d(1-\cos\theta_i)(1+\cos\theta_i),
 \qquad
 \kappa_n=\frac{2^{d^2}}{d!\,\pi^d}.
\]
Let $P_0(s)=1$, $P_1(s)=s$, and
$P_{k+2}(s)=sP_{k+1}(s)-P_k(s)$.  The Weyl integral form of Gessel's identity,
together with the finite transform in Equation~\eqref{eq:substitution}, gives
the exact identities
\[
 u_{n,k}=\kappa_n\int_{[0,\pi]^d}S_n(\theta)^kW_n(\theta)\,d\theta,
 \qquad
 b_{n,k}=\kappa_n\int_{[0,\pi]^d}P_k(S_n(\theta))W_n(\theta)\,d\theta.
\]
The second identity is coefficientwise: the recurrence for $P_k$ is exactly
the finite Fibonacci transform induced by the rational substitution.

The positive maximum of $S_n$ is $n$, attained at $\theta=0$.  With
$\theta=x/\sqrt{k}$, uniformly for $x$ in compact subsets of the positive
orthant,
\[
 \left(\frac{S_n(x/\sqrt{k})}{n}\right)^k
   \longrightarrow \exp\!\left(-\frac{\lVert x\rVert^2}{n}\right).
\]
For $s>2$, put $a(s)=(s+\sqrt{s^2-4})/2$.  The recurrence gives
\[
 P_k(s)=\frac{a(s)^{k+1}-a(s)^{-k-1}}{\sqrt{s^2-4}},
 \qquad
 \frac{d}{ds}\log a(s)=\frac1{\sqrt{s^2-4}}.
\]
Hence
\[
 \frac{P_k(S_n(x/\sqrt{k}))}{P_k(n)}
   \longrightarrow
   \exp\!\left(-\frac{\lVert x\rVert^2}{\sqrt{n^2-4}}\right).
\]

Under the same scaling, the Weyl weight and the Jacobian contribute
$k^{-\beta_n}Q_n(x)\,dx$, where $Q_n$ is a nonnegative homogeneous polynomial
of degree $2\beta_n-d$.  The elementary quadratic bounds for $1-\cos t$ and
the concavity of $\log a(S_n(\theta))$ near zero give an integrable Gaussian
envelope, so dominated convergence applies.  Outside a fixed neighborhood of
zero and, in even rank, the all-minus point, $|S_n|$ is bounded away from $n$
and the contribution is exponentially smaller.  At the even-rank all-minus
point, the factor $\prod_i(1+\cos\theta_i)$ vanishes and makes that
same-scale contribution lower order.  This also covers the one-dimensional $n=3$ case,
where the negative endpoint has absolute scale $1<3$.

Define
\[
 I_n(c)=\int_{\mathbb R_+^d}e^{-c\lVert x\rVert^2}Q_n(x)\,dx.
\]
The preceding limits and Equation~\eqref{eq:regev-leading} give
\[
 \kappa_n I_n(1/n)=C_n,
 \qquad
 \frac{b_{n,k}}{P_k(n)k^{-\beta_n}}
   \longrightarrow \kappa_n I_n(1/\sqrt{n^2-4}).
\]
Since $Q_n$ has degree $2\beta_n-d$, scalar change of variables yields
\[
 \frac{I_n(1/\sqrt{n^2-4})}{I_n(1/n)}
 =\left(\frac{\sqrt{n^2-4}}{n}\right)^{\beta_n}.
\]
Finally,
\[
 P_k(n)\sim\frac{\alpha_n^{k+1}}{\sqrt{n^2-4}}.
\]
...
\[
 ...
 \left(\frac{\sqrt{n^2-4}}{n}\right)^{\beta_n-1},
\]
which is Equation~\eqref{eq:fixed-rank-asymptotic}.
\end{proof}
...
\begin{proof}
The hook-length sum gives the bounded-height tableau formula
\[
 u_{4,2r}=C_rC_{r+1},\qquad u_{4,2r+1}=C_{r+1}^2.
\]
Substitution in Theorem~\ref{thm:main} gives
Equation~\eqref{eq:n-four-sum}.
...
involution has equal insertion and recording tableaux.  Thus
